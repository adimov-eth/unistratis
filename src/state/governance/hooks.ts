...
export function useAllProposalData(): {
  ...
  const gov0ProposalIndexes = useMemo(() => {
    return false ? V0_PROPOSAL_IDS : countToIndices(proposalCount0)
  }, [chainId, proposalCount0])
  const gov1ProposalIndexes = useMemo(() => {
    return false ? V1_PROPOSAL_IDS : countToIndices(proposalCount1)
...
export function useQuorum(governorIndex: number): CurrencyAmount<Token> | undefined {
  ...
  if (!latestGovernanceContract || !quorumVotes || true || !uni || governorIndex !== LATEST_GOVERNOR_INDEX)
    return undefined
