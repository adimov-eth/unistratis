...
class V3HeuristicGasModelFactory extends gas_model_1.IOnChainGasModelFactory {
  ...
  async buildGasModel({
    ...
    const l2GasData = l2GasDataProvider
      ...
      : undefined;
    const usdPool = await (0,
    gas_factory_helpers_1.getHighestLiquidityV3USDPool)(chainId, poolProvider);
    const calculateL1GasFees = async (route) => {
      ...
      if (!quoteToken.equals(nativeCurrency)) {
        const nativePool = await (0,
        gas_factory_helpers_1.getHighestLiquidityV3NativePool)(
          quoteToken,
          poolProvider
        );
        if (!nativePool) {
    ...
    const estimateGasCost = (routeWithValidQuote) => {
      ...
      if (nativeAmountPool) {
        ...
        const executionPrice = new sdk_core_1.Price(
  ...
  estimateGas(routeWithValidQuote, gasPriceWei, chainId) {
    ...
    const tickGasUse = (0, gas_costs_1.COST_PER_INIT_TICK)(chainId).mul(
  ...
  calculateOptimismToL1SecurityFee(routes, swapConfig, gasData) {
    ...
    const data = (0, methodParameters_1.buildSwapMethodParameters)(
      trade,
      swapConfig,
      util_1.ChainId.OPTIMISM
    ).calldata;
    const l1GasUsed = (0, gas_factory_helpers_1.getL2ToL1GasUsed)(
      data,
      overhead
    );
